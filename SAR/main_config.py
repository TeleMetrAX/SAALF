...
# that file, without editing this one.
...
# by SAR at the parameters below, with no false positives. NOTE: the NEK,
# NAB, Yahoo, IOPS and CESNET-TimeSeries24 data referenced throughout this
# file are not included in this repository - see the README for where to
# obtain them and how to lay them out under data/.
...
# mode except tune/tune-point-wise/tune-combined.
WIDE_INPUT = "data/cesnet_ip1367_n_packets_hourly.csv"
WIDE_VALUE_LABEL = r"Packets per hour"
WIDE_CWDE_SIGNALS_PATH = "data/C-WDE/cesnet_1367_n_packets_cwde.csv"
...
# precision/recall (F1 < 0.1). NEK_51/56, NEK_41/46, NEK_6/1 and NEK_36/31
# are each duplicate pairs of the same underlying series.
# EVAL_EXCLUDE_FILES = [
...
# better than they do with NEK, so --mode tune-point-wise tunes each of the
# three separately (see tuning.py) and writes its results straight into
# sar_params.json's "pointwise" entries.
...
#    detection quality; this mode exists to see/quantify that directly,
#    not as a real tuning target.
...
#    well within TUNE_*_RANGE) and pull against each other through G_MAX
#    (merging trades count for duration), which let this reward collapse
#    to signalling nothing at all rather than balancing the two; coverage
#    fraction folds both into one number that's naturally bounded in
#    [0, 1] - it can never exceed "the whole timeline" - so it needs no
#    baseline normalisation to sit on the same scale as F1.
...
# washed out at high G_MAX, so even multivariate TPE benefits from
# starting with direct evidence of it rather than having to find it by
# chance. All four points score well on NEK (mean F1 0.5418, 0.5584,
# 0.5544, and 0.5596 respectively; re-verify if NEK's data or SAR itself
# changes).
TUNE_SEED_POINTS = [
