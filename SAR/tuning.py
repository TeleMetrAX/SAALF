...
Optuna, differing only in which datasets they score and how:
  ...
  - --mode tune-point-wise: the same mean-F1 reward, but run as three
    ...
    with each other any better than with NEK. Their ground truth is individual points rather than
  ...
  - --mode tune-combined: both groups at once, in a single two-term
    ...
    other through G_MAX and are otherwise unbounded) over the point-wise datasets, weighted by
...
moderate G_MAX and washes out at high G_MAX, so a sampler that ignores that interaction can converge on
...
def pointwise_coverage_over_datasets(datasets, period_seconds, smoothing_window, lam, g_max, d_min):
    ...
    raw mean duration on their own, which have no upper bound - so it
    needs no baseline normalisation to sit on the same scale as F1.
    datasets: {name: [(t, x, true_labels),
    ...]}.
...
def _make_study():
    ...
    already long enough that pruning barely bites regardless of D_MIN - so
    the default univariate sampler can settle on "G_MAX high, D_MIN irrelevant" without ever revisiting
...
def run_tune_pointwise(args):
    ...
    each other any better than they do with NEK). F1 structurally collapses on point-wise ground truth
