...
SAR is the sustained-anomaly-recognition component of [SAALF](../README.md), a Semi-Automatic Anomaly Labelling Framework for univariate time-series data. It detects **sustained anomalous regions** in a periodic (e.g. daily) univariate time series — stretches of time where the data drifts away from its usual recurring pattern, as opposed to single-point spikes, which SAALF's other component (C-WDE) targets instead.
...
- `data/` — **not included in this repository** (see "Data" below); the expected folder layout for the labelled benchmarks and the CESNET example series.
- `results/` — output of a run (created by `main.py`).

## Data

None of the datasets used in the SAALF paper's experiments are bundled in this repository - they are third-party benchmarks that aren't ours to redistribute. To reproduce the paper's results, obtain each dataset from its original source and lay it out under `data/` as follows (all paths below are the defaults in `main_config.py`; every one of them can be overridden, either by editing that file or via CLI flags):

- `data/NAB/` — the [Numenta Anomaly Benchmark](https://github.com/numenta/NAB), labelled CSVs matching `labeled_*.csv` with a binary `label` column.
- `data/Yahoo/` — the Yahoo Webscope S5 A1 ("real") subset, labelled CSVs matching `labeled_Yahoo_*_data.csv` with a binary `label` column ("S5 - A Labeled Anomaly Detection Dataset"). Yahoo's Webscope catalog site (`webscope.sandbox.yahoo.com`) is currently unreachable; request access by emailing `research-data-requests@yahoo-inc.com` with the dataset name, your affiliation, and intended research use (per Yahoo's Webscope Data Sharing Agreement process).
- `data/IOPS/` — the IOPS KPI anomaly-detection benchmark (as repackaged by the [TSB-UAD benchmark suite](https://github.com/TheDatumOrg/TSB-UAD)), labelled CSVs matching `labeled_IOPS_*.csv` with a binary `label` column.
- `data/NEK/` — the NEK (Network Equipment KPI) dataset, labelled CSVs matching `labeled_NEK_*_data.csv` with a binary `label` column; available in the `NEK/` folder of [CSTCloudOps/Dataset-for-TSAD](https://github.com/CSTCloudOps/Dataset-for-TSAD).
- `data/cesnet_ip1367_n_packets_hourly.csv`, `data/cesnet_ip1367_n_flows_hourly.csv`, `data/cesnet_ip1367_n_bytes_hourly.csv` — hourly `n_packets`/`n_flows`/`n_bytes` series for one IP address, extracted from [CESNET-TimeSeries24](https://zenodo.org/records/13382427), each with a `timestamp` and `value` column.
- `data/C-WDE/` — **included** (unlike every other dataset above, this is our own derived output, not third-party data): `cesnet_1367_n_packets_cwde.csv`/`n_flows`/`n_bytes`, C-WDE's point-wise anomaly signal (`cwde_alert` column) for the CESNET series, row-aligned by timestamp, used for the `WIDE_CWDE_SIGNALS_PATH` overlay in `--mode single-wide`/`cesnet`. The raw `n_packets`/`n_flows`/`n_bytes` CESNET series themselves still need to be supplied separately (see above) for the overlay to have something to align against.

See the paper for exact dataset citations and the subsets used.
